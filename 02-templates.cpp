...
int main_templates(int argc, char * argv[]) {
	int i = 10;
	const int ci = 15;
	const int &cir = ci;
	int * pi = &i;
	...
	//func1({ 1 }); // 1.7. param type			could not deduce template argument 
	//func1({ 1, 2, 3 }); // 1.8. param type	could not deduce template argument 
	func2(i); // 2.1. param type				int
	func2(ci); // 2.2. param type				const int
	func2(cir); // 2.3. param type				const int
	//func2(42); // 2.4. param type				cannot convert rvalue to int&
	func2(pi); // 2.5. param type				int*
	func2(cpi); // 2.6. param type				const int*
	//func3(i); // 3.1. param type				cannot convert int to int*
	//func3(ci); // 3.2. param type 			cannot convert int to int*
	//func3(cir); // 3.3. param type 			cannot convert int to int*
	//func3(42); // 3.4. param type 			cannot convert int to int*
